let rawInput = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
const rawInput1 = `gea agfcbe egcbfd aecb cegbf cafgde fgeba ea gabdf begfdca | bcfeg ea fbdga fbgcdae
...
const UNIQUE_SEGMENTS = [2, 3, 4, 7]

let data = rawInput1
  .split('\n')
  .map((line) => line.trim().split('|'))
  .map((pair) => {
    const [inputs, outputs] = pair
...
function countUniques() {
  let count = 0
  for (let [, outputs] of data) {
    for (let o of outputs) {
      if (UNIQUE_SEGMENTS.includes(o.length)) {
        count++
      }
    }
  }
...
function decodeOutputValues(input, output) {}

function sumAllOutputs() {
  let result = 0
  for (let [inputs, outputs] of data) {
    let outputValues = decodeOutputValues(inputs, outputs)
    result += outputValues.array.forEach((element) => {
      result += element
    })
  }
  return result
...
function solution() {
  const uniques = countUniques()
  document.getElementById('answer_part_1').innerHTML = `${uniques}`
  const total = sumAllOutputs()
  document.getElementById('answer_part_2').innerHTML = `${total}`
}
